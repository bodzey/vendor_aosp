...
- githubremote:    Add git remote for RohieOS Github.
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/RohieOS-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.rohie.device | grep -q "$CUSTOM_BUILD"); then
...
function githubremote()
{
    ...
    git remote add github https://github.com/RohieOS/$PROJECT
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.rohie.device | grep -q "$CUSTOM_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell getprop ro.rohie.device | grep -q "$CUSTOM_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
